<script setup>
import { getCheckoutAPI, submitOrderAPI } from '@/api/checkout'
...
const submitOrder = async () => {
  const res = await submitOrderAPI({
    ...
    deliveryTimeType: deliveryIndex.value + 1,
    payType: payIndex.value + 1,
    payChannel: 1,
    buyerMessage: ''
  })
    ...
    <div class="container">
      <div class="wrapper">
        ...
        <div class="box-body">
          <div class="address">
            <div class="text">
              <div class="none" v-if="!curAddress.id">
                您需要先添加收货地址才可提交订单。
              </div>
              <ul v-else>
                <li>
        ...
        <div class="box-body">
          <a
            ...
            :key="index"
            @click="deliveryIndex = index"
            :class="{ active: deliveryIndex === index }"
            >{{ item }}</a
          >
        </div>
        <!-- 支付方式 -->
  ...
  <el-dialog v-model="toggleFlag" title="切换收货地址" width="30%" center>
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="handleToggleConfirm">确定</el-button>
      </span>
    </template>
  </el-dialog>
  <!-- 添加地址 -->
